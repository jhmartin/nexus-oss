    ...
    extends TestSupport
{
  ...
  public void simpleTimestamp() throws Exception {
    ...
    data.put("a", "a");
    defaultNexusTimeline.add(new Entry(System.currentTimeMillis() - Time.hours(1).toMillis(), "TEST", "1", data));
    data.put("b", "b");
    defaultNexusTimeline.add(new Entry(System.currentTimeMillis() - Time.hours(1).toMillis(), "TEST", "2", data));

    List<Entry> res;

    // 1st possible hit skipped, so empty results expected
    res = asList(1, 10, Collections.singleton("TEST"), Collections.singleton("1"), null);
  ...
  public void order() throws Exception {
    final Map<String, String> data = Maps.newHashMap();
    data.put("place", "2nd");
    defaultNexusTimeline.add(new Entry(System.currentTimeMillis() - Time.hours(2).toMillis(), "TEST", "1", data));
    data.put("place", "1st");
    defaultNexusTimeline.add(new Entry(System.currentTimeMillis() - Time.hours(1).toMillis(), "TEST", "1", data));
